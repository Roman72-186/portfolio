...
def cycle_for_day(db: Session, user_id: int, day: date) -> LearningTopic | None:
    ...
    Отвечает на календарный вопрос «какой цикл идёт в этот день» — не на
    вопрос «где ученик стоит сейчас», это `effective_cycle` ниже, и при
    пересечении периодов две функции отвечают по-разному намеренно (13.09.2026;
    до этого расхождение не было описано и не было покрыто тестами).
...
def effective_cycle(db: Session, user_id: int, today: date) -> LearningTopic | None:
    ...
    При пересечении периодов отдаёт **раньше начавшийся** из незакрытых —
    противоположно `cycle_for_day`, и это не оплошность: долг важнее новизны,
    иначе начало нового цикла молча прятало бы несданное из предыдущего.
    Совпадение с `cycle_for_day` наступает только когда долгов нет — тогда
