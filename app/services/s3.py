...
def s3_path_guest_feedback(submission_id: int, filename: str) -> str:
    """Видео/голосовое обратной связи гостевой сдачи: feedback-guest/{submission_id}/{random}.ext."""
    ext = filename.rsplit(".", 1)[-1].lower() if "." in filename else "jpg"
    rnd = uuid.uuid4().hex[:8]
    return f"feedback-guest/{submission_id}/{rnd}.{ext}"
