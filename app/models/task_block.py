...
# BLOCK_TYPES: владелец 16.09.2026 попросил убрать кнопки «+ Текст» и
# «+ Фото» — отдельные текст и фото вытеснил блок «Фото + сдача работы», а
# пояснение к заданию пишется в его описании.
#
# Почему отдельный кортеж, а не вычеркнуть типы из BLOCK_TYPES: блоки этих
# двух типов уже стоят в сохранённых заданиях. Убери их из BLOCK_TYPES — и
...
BLOCK_TYPES_ADDABLE = tuple(
    t for t in BLOCK_TYPES if t not in (BLOCK_TEXT, BLOCK_PHOTO)
)
...
BLOCK_TYPE_LABELS = {
    ...
    # 16.09.2026). Подпись одна на три места: кнопка конструктора, заголовок
    # карточки блока в редакторе и запасное название блока в очереди проверки
    # у куратора (`task_blocks.list_submissions_for_review`).
