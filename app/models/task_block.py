...
# BLOCK_PORTFOLIO в обоих списках с 17.09.2026 (владелец, после созвона
# 16.09.2026: «в кнопку «Загрузить портфолио» добавить видеоинструкцию,
# фотографии, скриншоты, а ниже — сама кнопка»). Видео и фото у кнопки
# необязательны: блок по-прежнему самодостаточен.
#
...
# в его описании. Фото снова доступно с 17.09.2026: преподавателю нужны
# примеры работ внутри инструкции, а «Домашнее задание» (см. ниже) решает
# другую задачу и не заменяет информационную галерею.
#
# BLOCK_UPLOAD снят с 17.09.2026 (владелец на созвоне 16.09.2026: «слить
# фотоблок и блок «Домашнее задание» в конструкторе в один тип»). Кнопка
# приёма работы без галереи-задания больше не заводится — вместо неё
# используется BLOCK_PHOTO_UPLOAD, принявший ту же подпись «Домашнее
# задание» (см. BLOCK_TYPE_LABELS). Значение не совпадает с восстановлением
# фото: это два независимых решения, случайно совпавших датой.
#
# Почему отдельный кортеж, а не вычеркнуть типы из BLOCK_TYPES: блоки этих
# типов уже стоят в сохранённых заданиях. Убери их из BLOCK_TYPES — и
# валидация начнёт отбивать сохранение старого задания 400-й, а карточка
# блока в редакторе потеряет подпись. Скрыта только кнопка «завести новый».
#
# Список собирается фильтрацией, а не перечислением: порядок BLOCK_TYPES и
# есть порядок кнопок, и новый тип должен попадать в конструктор сам.
BLOCK_TYPES_ADDABLE = tuple(
    t for t in BLOCK_TYPES if t not in (BLOCK_TEXT, BLOCK_UPLOAD)
)
...
BLOCK_TYPE_LABELS = {
    ...
    # «Домашнее задание» вместо прежнего «Загрузить работы» (владелец
    # 16.09.2026). Подпись остаётся у старых блоков этого типа — сама кнопка
    # добавления снята 17.09.2026 (см. BLOCK_TYPES_ADDABLE), значение в базе
    # не менялось.
    BLOCK_UPLOAD: "Домашнее задание",
    BLOCK_RULES: "Правила с галочками",
    # Единственный оставшийся добавляемый тип для сдачи работы (владелец
    # 16.09.2026: слить фотоблок и «Домашнее задание» в один тип) — подпись
    # сменилась с «Фото + сдача работы» на ту же, что у BLOCK_UPLOAD.
