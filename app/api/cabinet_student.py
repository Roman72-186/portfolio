...
# Сохранённый год за пределами списка добавляем отдельным пунктом: staff мог
# проставить год через карточку ученика (cabinet_students_shared) до того, как
# анкета заполнена, и без этого пункта сохранение анкеты затрёт его значение.
