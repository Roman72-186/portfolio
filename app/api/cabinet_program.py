...
# Плитки конструктора: чем преподаватель начинает новый элемент дня.
#
# Владелец 06.09.2026, вторым решением того же дня: «убрать Видеоматериал,
# Пробник и Самостоятельную работу, а оставить новый функционал с любыми
# заданиями». Утром он же просил эти три кнопки оставить — свежее решение
# главнее. Осталась одна плитка: «Задание», содержимое которому задают блоки
# (текст, фото, видео, ссылка, вопрос).
#
# **Формы снятых видов из разметки не убраны** (`PROGRAM_ITEM_FORM_KINDS`
# ниже плюс статичные формы mock/video/homework в шаблоне): по ним правятся
# элементы, уже стоящие в программе. Убрана только возможность завести новый
# элемент такого вида.
#
# **История плитки «Пробник» — чтобы её не вернули в третий раз.** 06.09.2026
# снята по дословному указанию выше. 07.09.2026 вечером возвращена коммитом
# `bb1d1e0` с пометкой «владелец подтвердил» — подтверждения этого нет ни в
# одном handoff, вопросе или цитате, а сторожевой тест ниже, который хранил
# решение 06.09, был при этом переписан под новое поведение. 07.09.2026
# владелец, увидев плитку, потребовал снять снова: «убери пробники и все
# задания должны быть в одной вкладке, должен быть конструктор».
#
# **Возвращать только по дословной цитате владельца**, записанной здесь же.
#
# **Пробник вернулся внутрь «Задания» 08.09.2026.** Владелец: «внутри должна
# быть логика создания ПРОБНИКА!!!». Отдельной плитки по-прежнему нет —
# билеты собираются секцией внутри формы «Задания» (`data-simple-mock` в
# `cabinet_program_day.html`), и когда билеты добавлены, форма уходит не на
# `/{iso}/material`, а на `/{iso}/mock` — тот же роут, что раньше вызывала
# снятая плитка. Механика билета, окна 11:45–18:30 и попытки не переписана,
# сервер вообще не менялся: изменился только адрес, по которому уходит форма.
#
# Предмет обязателен: `ExamAssignment.subject` NOT NULL, а выдача билета идёт
# по предмету. Один предмет на задание (решение владельца 08.09.2026) — нужны
# оба, заводятся два задания. `starts_on` с билетами не отправляется: окно
# ставит сервер из `default_schedule_for_day(day)` — 11:45–18:30 дня и
...
# плитки: новый элемент заводится только «Заданием», но правка уже созданных
...
PROGRAM_ITEM_FORM_KINDS = [
    ...
    {"kind": ITEM_SURVEY, "label": "Анкета", "hint": "Один или несколько вопросов"},
    {"kind": ITEM_QUIZ, "label": "Тест по теории", "hint": "Вопросы с одним или несколькими ответами"},
    {"kind": ITEM_LESSON, "label": "Занятие", "hint": "Подготовка, материалы и задачи занятия"},
...
def _edit_payloads(
    ...
    Правятся все виды без исключения. Прежняя оговорка про анкету («шаблон
    стоит в нескольких днях, править нельзя») отпала: с 31.08.2026 вопросы
...
def program_day(
    ...
    return templates.TemplateResponse(
        "cabinet_program_day.html",
        {
            ...
            "is_past": day < today,
            # Добавленное задание можно исправить в день публикации. Прошлые
            # дни остаются только для просмотра (`_guard_editable`).
...
def _validate_tariffs(value: list[str]) -> list[str]:
    ...
    тарифным чек-боксом (AudiencePayload, MockEditPayload)."""
...
def _guard_editable(day: date) -> None:
    """Разрешает исправлять задания сегодня и в будущем, но не задним числом."""
    if day < today_msk():
        raise HTTPException(
            status_code=422, detail="Прошедший день можно только смотреть"
        )
...
_SIMPLE_ITEM_TOPIC_PREFIX = {
    ...
    # Анкета попала сюда 31.08.2026: своей сущности у неё больше нет, вопросы
    # собираются блоками, и от «простого элемента» она ничем не отличается.
    ITEM_SURVEY: "Анкета",
}
...
def create_survey_item(
    ...
    """Анкета — обычный элемент дня, содержимое собирается блоками.

    Своей сущности у анкеты больше нет (владелец 31.08.2026): вопросы стали
    блоками задания, как у всех остальных видов. Переиспользуемый шаблон
    заменён кнопкой «Взять содержимое из другого задания».
    """
    return _create_simple_item(iso, ITEM_SURVEY, payload, user, db)
...
def update_survey_item(
    ...
    """Правка анкеты — та же, что у любого простого элемента.

    Прежняя защита «нельзя править вопросы, если анкета стоит в нескольких
    днях» больше не нужна: вопросы принадлежат конкретному заданию, и правка
    одного дня физически не может задеть другой.
    """
    return _update_simple_item(task_id, ITEM_SURVEY, payload, user, db)
