...
# элемент такого вида.
#
# **Пробник вернули 07.09.2026.** Снос плитки 06.09 не тронул саму механику:
# роут `POST /cabinet/staff/program/{iso}/mock_exam`, форма `data-form="mock"`
# и вся её JS-обвязка остались на месте — не работала только кнопка, с которой
# всё начинается. Выяснилось при подсчёте, что стоит «сделать пробник блоком»:
# оказалось, делать нечего, надо вернуть строку. Владелец подтвердил 07.09.
#
# Видеоматериал и Самостоятельная работа не возвращены намеренно: ролик
# ставится блоком «Видео», приём работ — блоком «Загрузить работы»
# (07.09.2026). У пробника своей замены нет: билет, окно 11:45–18:30 и попытка
# блоками не собираются.
PROGRAM_ITEM_PRESETS = [
    ...
    {"kind": "mock", "label": "Пробник", "icon": "★", "hint": "Билеты по рисунку и композиции, окно сдачи", "capability": "mock", "default_block": None},
]
