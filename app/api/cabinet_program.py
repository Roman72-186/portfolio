...
#
# Следствие, известное и принятое: поставить в день **новый** пробник с
# билетом и таймером теперь нечем — блоками билет, окно 11:45–18:30 и попытка
# не собираются. Уже стоящие в программе пробники работают и правятся: снята
# кнопка создания, не механика. В предобучении 18–29 сентября пробника нет
# (разбор созвона 03.09), так что до октября это ничего не ломает.
...
PROGRAM_ITEM_FORM_KINDS = [
    {"kind": ITEM_MATERIAL, "label": "Задание", "hint": "Любое содержимое: текст, фото, видео, ссылка, вопросы"},
