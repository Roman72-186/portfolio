...
#
# 03.09.2026 — поднято с 35 до 61: завели тёмную тему, токены на тёмный режим
# живут во ВТОРОМ блоке `:root[data-theme="dark"] { ... }` (сам механизм CSS
# custom properties не даёт двум условным значениям одного токена ужиться в
# одном плоском :root). `css.index(":root")` в тесте ниже находит только
# первое, светлое, вхождение — все hex дальше по файлу (второй root-блок и
# пять точечных переопределений плашек `.tag-pill--*` под тёмную тему)
# помечаются «вне root», хотя по смыслу это тот же единый источник цвета,
# просто со вторым сценарием. Не забытые токены, не разбредшийся дизайн —
# ограничение самого замера.
BASE_CSS_HEX_OUTSIDE_ROOT = 61
