...
def test_day_page_uses_school_day_copy_and_inline_optional_hints(
    ...
    for placeholder in (
        ...
        "Текст",
        # У каждого типа блока — название и описание, первыми и в этом порядке
        # (владелец 25.09.2026, инвариант в AGENTS.md). До этого дня у фото
        # была «Подпись, необязательно», у видео и голосового «Заголовок,
        # необязательно», у ссылки «Надпись на кнопке» — и все они стояли
        # последними, после содержимого блока.
        "Название, необязательно",
        "Описание, необязательно",
        # У ссылки название работает ещё и надписью на кнопке — одно поле на
        # оба назначения, второй колонки в БД под это не заводили.
        "Название, оно же надпись на кнопке",
        "Адрес ссылки",
        "Текст вопроса",
    ):
